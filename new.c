...
#pragma config(Sensor, in2,    a,              sensorPotentiometer)
#pragma config(Sensor, in3,    b,              sensorPotentiometer)
#pragma config(Sensor, in4,    GyroX,          sensorGyro)
#pragma config(Sensor, in5,    GyroY,          sensorGyro)
#pragma config(Sensor, in6,    GyroZ,          sensorGyro)
#pragma config(Sensor, dgtl1,  ,               sensorSONAR_mm)
...
#pragma config(Motor,  port9,           GlMotor,       tmotorVex393_MC29, openLoop, encoderPort, dgtl3)
...
task Read()
{
	...
	{
		//Sensor
		int S = SensorValue[dgtl3];
		wait1Msec(100);
		int E = SensorValue[dgtl3];
		V = (E-S)*10;
...
task Do()
{
	...
	while(true)
	{
		//tt = SensorValue[b]-SensorValue[a];
		//setMotorTarget(DtMotor, tt, 64, false );
...
task Decision()
{
	while(true)
	{
		...
		if(abs(up)<64)
		{
			...
			Command.LeftMotor = up - 2.5 * turn;
			Command.RightMotor = up + 2.5 * turn;
		}
	}
}
